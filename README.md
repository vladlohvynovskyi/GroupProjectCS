Welcome to the Dreadful Depths! The main objective for the game is to get as deep into the dungeon as possible while ensuring a fast and efficient journey. It is filled with NPCs and dangerous enemies, so beware! You might also just find some objects laying around that might help you along your journey. All the best and don't dread the depths too much (they might just take revenge).

Follow the steps below to run the game on your device(make sure you have python3 installed):
1. Create a virutal environment using: python3 -m venv .venv
2. Activate it using: source .venv/bin/activate
3. Install pygame using: pip install pygame
4. Go to the folder/directory where are the game files are stored using: cd {your_folder_name}
5. Rung the game using: python3 main.py
6. Enjoy!
   

# GroupProjectCS


Assets:

https://bakudas.itch.io/generic-dungeon-pack

https://snowhex.itch.io/dungeon-gathering

https://pixel-poem.itch.io/dungeon-assetpuck

https://0x72.itch.io/dungeontileset-ii

https://humblepixel.itch.io/pocket-inventory-series-5-player-status

https://admurin.itch.io/enemy-golem-reinforced

https://flavrius.itch.io/scary-mobs-aaahhh

https://glionox.itch.io/items16#google_vignette

https://stext25.itch.io/cherry-blossom

https://fonts.google.com/specimen/MedievalSharp?preview.script=Latn
